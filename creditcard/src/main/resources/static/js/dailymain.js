...
var w_fes = new Array("0520 母亲节", "0630 父亲节");
var today = new Date();
...
function calendar(y,m) {
   ...
   for(var i=0;i<this.length;i++) {
      ...
      //if((i+this.firstWeek)%7==0)   this[i].color = '#fb493e'//星期日
     // if((i+this.firstWeek)%8==0) this[i].color = '#ff0'//星期六
   }
   ...
   //if(m==3) this[t_1].color = '#fb493e'//清明节
   ...
   for(i in w_fes)
      if(w_fes[i].match(/^(\d{2})(\d)(\d)([\s\*])(.+)$/))
         if(Number(RegExp.$1)==(m+1)) {
            t_1=Number(RegExp.$2)
            t_2=Number(RegExp.$3)
            if (this[((this.firstWeek>t_2)?7:0) + 7*(t_1-1) + t_2 - this.firstWeek] && this[((this.firstWeek>t_2)?7:0) + 7*(t_1-1) + t_2 - this.firstWeek].solar_festival) {
                this[((this.firstWeek>t_2)?7:0) + 7*(t_1-1) + t_2 - this.firstWeek].solar_festival += RegExp.$5 + ' ';
            } else {
                this[((this.firstWeek>t_2)?7:0) + 7*(t_1-1) + t_2 - this.firstWeek] = {
                    solar_festival: RegExp.$5 + ' '
                }
            }
         }
   for(i in lunar_fes)
     //lunar_fes.style.color="#f30000";
      if(lunar_fes[i].match(/^(\d{2})(.{2})([\s\*])(.+)$/)) {
         ...
         if(t_1 >=0 && t_1<n) {
            ...
            if( t_2 >= 0 && t_2<this.length) {
               ...
               /*if(RegExp.$3=='*') this[t_2].color = '#fb493e'//农历节日*/
			    if(RegExp.$3=='*'){
					this[t_2].color = '#fb493e'//农历节日
				}
				//this[t_2].color = '#fb493e'//农历节日
            }
         }
      ...
      if(solar_fes[i].match(/^(\d{2})(\d{2})([\s\*])(.+)$/))
         ...
         if(fes == "") continue;
            ...
            if(RegExp.$3=='*') this[Number(RegExp.$2)-1].color = '#fb493e'//元旦、劳动。国庆
         }
...
function setCld(SY,SM) {
   ...
   for(i=0;i<42;i++) {
      ...
      if(sD>-1 && sD<cld.length) {
         ...
         if(cld[sD].istoday){
               solar_obj.style.background = '#ffa500';
			   solar_obj.style.color = '#fff';
               lunar_obj.style.background = '#ffa500';
			   lunar_obj.style.color = '#fff';
               // alert(i);
               addDay(i);
         }

         if(cld[sD].l_d==1)
            lunar_obj.innerHTML = '<b>'+(cld[sD].isLeap?'闰':'') + cld[sD].l_m + '月' + (lunar_leap_d(cld[sD].lYear,cld[sD].l_m)==29?'':'')+'</b>';
         else
         ...
         if(s.length>0) {
            ...
            if(s.length>0) {
               ...
               s = s.fontcolor('#fb493e');  //节日
            } else {
               s=cld[sD].solarTerms;//24节气
               if(s.length>0) s = s.fontcolor('#fb493e');
            }
         }
...
function addDay(v) {
   ...
   if(solar_obj.innerHTML!='') {
	  ...
	  //solar_obj.style.background = '#f30000';
      fes = '<table width="100%" ><tr><td>'+'<span><b>'+cld[d].solarTerms + ' ' + cld[d].solar_festival + ' ' + cld[d].lunar_festival+'</b></span></td>'+
                 '</tr></table>';
   ...
   var $dayitable = $(".dayimtimeboxcoent .daylistdate td");
   $dayitable.click(function(){
	   $(this).css({"background":"#ffa500","color":"#fff"});
	   $(this).parents(".dayimtimeboxcoent td").siblings().css({"background":"#fff","color":"#999"});
	   var index = $dayitable.index(this);
	    $(".dayimtimeboxcoent .daylistvin td").eq(index).css({"background":"#ffa500","color":"#fff"})
   })
}
